    ...
    componentDidMount() {
        ...
        axios.get(`/Pregunta?id=${qId}`)
            .then(response => {
                ...
                if (question) {
                    // columnajson puede llegar como objeto (lo normal con
                    // Sequelize) o como cadena: parseColumnaJson maneja ambos.
                ...
                <div className="DinámicaGrid">
                    {
                        targets.map((target, index) => {
                            return (
                                <span key={index} className="TarjetaOpción">
                                    {/* resolverImagen respeta URLs absolutas y antepone el backend a las relativas */}
